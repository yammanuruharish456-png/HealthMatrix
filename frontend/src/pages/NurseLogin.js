...
const NurseLogin = () => {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      login(res.data.token, res.data.user);
      toast.success('Nurse Google Sign-In successful!');
      navigate('/nurse-dashboard');
    } catch (error) {
    ...
    <div className="auth-page">
      <div className="auth-container">
        <div className="auth-card">
          <h2>Nurse Login</h2>
          <p className="auth-subtitle">Nursing Care Portal</p>
          ...
          <p className="auth-footer">
            Don't have an account? <Link to="/nurse-register">Register here</Link>
          </p>
          <p className="auth-link-row">
            <Link to="/login">Patient</Link> | <Link to="/doctor-login">Doctor</Link> | <Link to="/admin-login">Admin</Link> | <Link to="/receptionist-login">Receptionist</Link> | <Link to="/lab-technician-login">Lab Technician</Link>
          </p>
        </div>
      </div>
    </div>
