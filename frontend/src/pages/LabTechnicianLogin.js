...
const LabTechnicianLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.data.user.role !== 'lab_technician') {
        toast.error('Access denied. Lab Technician credentials required.');
        return;
      }
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      login(res.data.token, res.data.user);
      toast.success('Lab Technician Google Sign-In successful!');
      navigate('/lab-technician-dashboard');
    } catch (error) {
    ...
    <div className="auth-page">
      <div className="auth-container">
        <div className="auth-card">
          ...
          <p className="auth-subtitle">Laboratory Portal</p>
          ...
          <p className="auth-footer">
            Don't have an account? <Link to="/lab-technician-register">Register here</Link>
          </p>
          <p className="auth-link-row">
            <Link to="/login">Patient</Link> | <Link to="/doctor-login">Doctor</Link> | <Link to="/admin-login">Admin</Link> | <Link to="/receptionist-login">Receptionist</Link> | <Link to="/nurse-login">Nurse</Link>
          </p>
        </div>
      </div>
    </div>
