...
const DoctorLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.data.user.isVerified === false) {
        toast.error('Your account is pending admin verification. Please wait for approval.');
        return;
      }
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      login(res.data.token, res.data.user);
      toast.success('Doctor Google Sign-In successful!');
      navigate('/doctor-dashboard');
    } catch (error) {
    ...
    <div className="auth-page">
      <div className="auth-container">
        <div className="auth-card">
          <h2>Doctor Login</h2>
          <p className="auth-subtitle">Medical Professional Portal</p>
          <form onSubmit={handleSubmit}>
            ...
            <button type="submit" className="btn-primary">Login as Doctor</button>
          ...
          <p className="auth-footer">
            Don't have an account? <Link to="/doctor-register">Register as Doctor</Link>
          </p>
          <p className="auth-link-row">
            <Link to="/login">Patient</Link> | <Link to="/admin-login">Admin</Link> | <Link to="/receptionist-login">Receptionist</Link> | <Link to="/nurse-login">Nurse</Link> | <Link to="/lab-technician-login">Lab Technician</Link>
          </p>
        </div>
      </div>
    </div>
